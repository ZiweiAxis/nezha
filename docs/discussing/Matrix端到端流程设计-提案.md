...
> **关键说明**：注册流程与执行拦截流程不同：
> - **注册**：悟空注册模块和天枢都是**无感知**獬豸的，SDK在后台拦截并处理审批
> - **执行**：操作被拦截时需要显式审批
...
2. 天枢将请求转发给獬豸（经代理/鉴权）
       ↓
3. 獬豸拦截，策略判断需要审批（Review）
       ↓
4. 獬豸调用天枢投递 API（POST /api/v1/delivery/approval-request）
       ↓
...
- 獬豸固定 DID：`did:ziwei:local:xiezhi`（或类似格式）
- 启动时自动连接 Matrix（通过天枢）
- 需要解决授权问题：如何让獬豸自动接入天枢
...
**保持现有方案**：经天枢投递
- 獬豸调用 `POST /api/v1/delivery/approval-request`
- 天枢负责推送到 Matrix 房间
...
3. **獬豸启动授权**：獬豸启动时自动连接天枢，需要解决授权问题（如预共享密钥、配置 token 等）
