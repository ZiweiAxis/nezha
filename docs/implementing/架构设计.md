...
| **谛听 (Diting)** | 策略引擎 | 策略管理、风险评估、人工审批、审计存证 (PDP+PAP) | Go | MVP |
...
| 模块 | 位置 | 功能 |
|------|------|------|
| **谛听客户端** | 集成在悟空内部 | PEP 拦截、日志采集、策略请求 |
| **策略引擎** | diting/policy/ | 策略定义、存储、下发 |
| **决策引擎** | diting/proxy/ | 接收悟空请求，决策放行/拒绝/审批 |
| **审批模块** | diting/cheq/ | 构建审批单、CHEQ 管理 |
| **审计存储** | diting/audit/ | 日志接收与存储 |
| **消息投递** | diting/delivery/ | 审批消息投递给天枢 |
...
| DITING_API_URL | 谛听 API 地址 | http://localhost:8080 |
