...
- **范围**：
  ...
  - **太白**：提供协议与 SDK，并**默认提供**与该 CLI 的适配器（发现天枢、注册、心跳、指令收发、可选操作上报），不实现 CLI 内部逻辑；执行仍在 CLI，监听效果经獬豸机制达到。
...
| **獬豸** | 一套机制达到监听效果；CLI 侧执行若经獬豸（如 `/auth/exec`），则由獬豸机制鉴权/审批/审计。 |
...
- **太白适配器**：与 verification_agent 不同，不仅做「发现+注册+心跳+一条上报」，而是**持续联络**：接收天枢下发的任务，转成 Claude Code CLI 可接受的输入（如一条自然语言指令或 JSON 任务描述），驱动 CLI 执行，收集 stdout/stderr 或结构化结果，按天枢约定格式回传；可选将「操作类型」上报獬豸（如 `file_write`、`api_call`）。
...
3. **执行**  
   Claude Code CLI 在本地执行（读/写文件、执行命令等）。若执行命令前经獬豸 `/auth/exec`，则由适配器或 CLI 侧集成在执行前请求獬豸，按策略 allow/deny/review；执行仍由 CLI 完成。

4. **回传结果**  
   CLI 执行完毕，适配器收集输出（stdout/stderr 或 CLI 返回的结构化结果），按天枢约定的响应格式（如指定事件类型、载荷结构）经太白回传天枢。

5. **操作上报（可选）**  
   适配器可在关键操作后（如完成一次文件写入、一次命令执行）向獬豸上报 `m.agent.action` 或等价 HTTP，便于审计与 Trace。
...
| `DITING_AUDIT_URL` | 獬豸审计上报（可选） | 留空则不上报 |
...
说明：与验证用智能体共用同一套天枢/獬豸相关变量，通过不同示例目录或配置区分「仅验证」与「Claude Code CLI Agent」。
...
- **Claude Code CLI Agent（本方案）**：在 discovery/register/heartbeat 基础上，增加**持续接收天枢任务 → 驱动 CLI 执行 → 回传结果**，并可选上报操作至獬豸，作为太白**默认提供的可执行 Agent** 形态之一。
...
- **后续**：獬豸执行鉴权（如执行命令前调 `/auth/exec`）在适配器或 CLI 侧集成；多任务队列、超时与重试策略可在适配器内迭代。
