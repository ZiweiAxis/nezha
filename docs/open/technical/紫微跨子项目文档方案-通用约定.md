...
**基础规范**：所有子仓库在主仓中均为 **git submodule**。**BMAD 文档仍按 BMAD 规范存放在各子项目目录内**（如 `tianshu/_bmad-output/planning-artifacts/`、`diting/_bmad/`）；**Git 层面**由子仓 `.gitignore` 忽略、**由主仓跟踪并提交**这些路径。**悟空（wukong）** 及 **源于 wukong 的子项目**适用同一约定。
