...
| 模块 | 仓库/路径 | 职责 | 集成角色 |
|------|-----------|------|----------|
| **天枢** | ziwei/tianshu | 通信枢纽、身份、DID 注册/查询调用、企业 IM 审批触达 | 调用谛听（DID、初始化权限、审计）；被太白/智能体调用（发现、注册、心跳） |
...
**说明**：Matrix（Synapse）为天枢的通信依赖，当前由 tianshu 的 compose 编排；链为谛听内子模块，不单独列为一个「模块」。
...
| 审批回调 | 谛听/IM → 天枢 | 天枢暴露回调 URL，谛听/飞书卡片点「通过」后请求该 URL | 天枢：`DITING_APPROVE_CALLBACK_URL`（天枢自身地址） | 技术方案 §2.4 |
...
| 谛听 CHEQ 审批 | 人工/IM → 谛听 | `GET/POST /cheq/approve?id=xxx&approved=true|false` | 飞书卡片内链接指向谛听公网/内网基址 | 谛听 I-008 |
...
| 角色 | 关键环境变量 | 说明 |
|------|--------------|------|
| **谛听** | `proxy.listen_addr`、`chain.enabled`、飞书 app_id/secret、approval_user_ids 等 | 见 diting `config.example.yaml` |
...
| **短期** | 根侧集成用 docker-compose（Synapse+天枢+谛听），CI 可选的平台回归 | `docker-compose.integration.yml`、CI 步骤文档 |
