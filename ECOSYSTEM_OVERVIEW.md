...
**紫微 (Ziwei)** 是一套完整的企业级 AI 智能体治理基础设施，由四个核心组件组成：

| 组件 | 中文名 | 功能定位 | 语言 | 状态 |
|------|--------|----------|------|------|
| **Tianshu** | 天枢 | 消息枢纽 (PIP) | Python | 开发中 |
| **Diting** | 谛听 | 策略引擎 (PDP+PAP) | Go | MVP |
| **Wukong** | 悟空 | 执行控制器 (PEP) | TypeScript | 开发中 |
| **Taibai** | 太白 | SDK 适配器框架 | Python | MVP |
...
- **Tianshu (天枢)**: 消息枢纽 (PIP) - 开发中
- **Taibai (太白)**: SDK 适配器框架 - MVP
- **Diting (谛听)**: 策略引擎 (PDP+PAP) - MVP
- **Wukong (悟空)**: 执行控制器 (PEP) - 开发中
...
| **PDP** | Policy Decision Point | 策略决策点，评估策略 | 谛听 (Diting) |
| **PAP** | Policy Administration Point | 策略管理点，管理策略 | 谛听 (Diting) |
...
**关键原则**：悟空注册模块和天枢注册接口都是无感知谛听存在的。

- **悟空注册模块**：无感知谛听，发出注册请求后等待 → 成功返回
- **天枢注册接口**：无感知谛听，正常处理注册请求
- **谛听客户端SDK**：作为普通调用拦截，审批完成后放行
