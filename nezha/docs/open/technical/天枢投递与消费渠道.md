...
- **审批请求投递**：獬豸等调用 `POST /api/v1/delivery/approval-request`，天枢将审批请求发到配置的 **DELIVERY_ROOM_ID**（Matrix 房间）；请求体含 `request_id`、`title`、`description`、`trace_id`、`gateway_base_url` 等。
...
| 审批请求 | 獬豸调 `POST /api/v1/delivery/approval-request`，天枢投递到 **DELIVERY_ROOM_ID**；用户/子项目在对应 Matrix 房间内看到审批消息。 |
